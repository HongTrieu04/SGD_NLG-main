...
if __name__ == '__main__':
    ...
    df['ref'] = df['ref'].str.replace('<vstripe>','|') 
    df['pred'] = df['pred'].str.replace('<vstripe>', '|')
    df['sr'] = df['sr'].str.replace('<vstripe>', '|')
    record_df = df
    ...
    if args.wandb_project:
        ...
        domain_ser_df = domain_df.T.copy()
        domain_ser_df['domain'] = domain_ser_df.index.copy()
        domain_ser_df.columns = ['ser', 'bleu', 'domain']
        domain_ser_plot_tb = wandb.Table(dataframe=domain_ser_df[["domain", "ser"]], columns=["domain", "ser"])
        domain_bleu_plot_tb = wandb.Table(dataframe=domain_ser_df[["domain", "bleu"]], columns=["domain", "bleu"])
        run = wandb.init(project=args.wandb_project, name='test-'+args.exp_name, config=config_dict)
        run.log({
            'Eval Metric Table': metric_tb, 
            'Annotated Cases': annotated_cases_tb, 
            'Domain SER Table': domain_ser_tb,
            'Domain SER Plot': wandb.plot.bar(domain_ser_plot_tb, "domain", "ser", title='Domain SER plot'),
            'Domain BLEU Plot': wandb.plot.bar(domain_bleu_plot_tb, "domain", "bleu", title='Domain BLEU plot'),
            'BLEU Score': res_dict['bleu'],
            'SER Score': res_dict['ser'],
        })